...
/**
 * Build the full list of .gitignore entries including skill directories.
 * @returns {string[]}
 */
function buildGitignoreEntries() {
  const skillEntries = TOBARI_SKILLS.map((s) => `.claude/skills/${s}/`);
  // Insert skill entries after the infrastructure header block
  const infra = GITIGNORE_ENTRIES.slice(0, GITIGNORE_ENTRIES.indexOf(""));
  const rest = GITIGNORE_ENTRIES.slice(GITIGNORE_ENTRIES.indexOf("") + 1);
  return [...infra, ...skillEntries, "", ...rest];
}
