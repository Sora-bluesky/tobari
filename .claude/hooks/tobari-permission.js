 ...
 *
 * Implements notification channel and notification architecture.
...
const SAFE_BASH_PATTERNS = [
  ...
  [/^git\s+branch(?!\s+.*-[dD]\b)\b/i, "git branch\uFF08\u5B89\u5168\uFF09"],
  ...
  [/^git\s+checkout(?!\s+--\s+\.)\b/i, "git checkout\uFF08\u5B89\u5168\uFF09"],
  ...
  [/^git\s+stash(?!\s+(drop|clear))\b/i, "git stash\uFF08\u5B89\u5168\uFF09"],
  // git stash list — explicitly safe (read-only)
  [/^git\s+stash\s+list\b/i, "git stash list"],
  ...
  [/^pwsh\b/i, "PowerShell \u30B9\u30AF\u30EA\u30D7\u30C8"],
  [/^powershell\b/i, "PowerShell \u30B9\u30AF\u30EA\u30D7\u30C8"],
  // Test runners
  [/^pytest\b/i, "\u30C6\u30B9\u30C8\u5B9F\u884C"],
  [/^npm\s+test\b/i, "\u30C6\u30B9\u30C8\u5B9F\u884C"],
  [/^npm\s+run\s+test\b/i, "\u30C6\u30B9\u30C8\u5B9F\u884C"],
  [/^python\s+-m\s+pytest\b/i, "\u30C6\u30B9\u30C8\u5B9F\u884C"],
  ...
  // Safe file ops
  [/^(mkdir|touch|cp|mv)\b/i, "\u30D5\u30A1\u30A4\u30EB\u64CD\u4F5C"],
  // Python / Node execution
  [/^(python|python3|node)\s+/i, "\u30B9\u30AF\u30EA\u30D7\u30C8\u5B9F\u884C"],
  ...
  // Text processing
  [/^(jq|awk|sed|xargs|tr|cut)\b/i, "\u30C6\u30AD\u30B9\u30C8\u51E6\u7406"],
  ...
  [/^(export|source|cd)\b/i, "\u30B7\u30A7\u30EB\u64CD\u4F5C"],
  // Bash variable / conditional checks
  [/^(test|true|false|\[)\b/i, "\u6761\u4EF6\u78BA\u8A8D"],
];
...
function describeOperation(toolName, toolInput) {
  ...
  if (toolName === "Task") {
    return (
      "\u30B5\u30D6\u30A8\u30FC\u30B8\u30A7\u30F3\u30C8\u3092\u8D77\u52D5: " +
      (toolInput.description || "").slice(0, 60)
    );
  }
  return toolName + " \u3092\u5B9F\u884C";
}
