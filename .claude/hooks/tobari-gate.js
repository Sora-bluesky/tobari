 ...
 *
 * Implements allow/deny/ask rules and STG gate spec.
 *
 * Profile behavior:
...
function _getProjectRoot() {
  ...
  if (projectDir) {
    return tobariSession.canonicalPathKey(projectDir);
  }
  ...
  try {
    ...
    let result = fallback.replace(/\\/g, "/").replace(/\/+$/, "");
    if (_IS_WINDOWS) result = result.toLowerCase();
    return result;
  }
}
...
// --- Protected Directories (A1: v1.2.0) ---
...
const PROTECTED_DIRECTORY_EXCEPTIONS = [
...
function validateInput(filePath, content) {
  ...
  // Path traversal via canonicalPathKey normalization
...
/**
 * A1: Protected Directory Deny — block writes to governance infrastructure.
 *
 * Denies Edit/Write to .git/, .claude/hooks/, .claude/rules/, .agents/
 * UNLESS the file is explicitly included in the contract scope.
 * Exceptions: .claude/tobari-session.json and .claude/logs/ (Hook-writable).
 */
function checkProtectedDirectory(filePath, toolName) {
  // If explicitly included in contract scope, allow
  ...
  let relKey = fileKey;
  if (projectRoot && fileKey.startsWith(projectRoot + "/")) {
    relKey = fileKey.slice(projectRoot.length + 1);
  }

  // Check exceptions first
  for (const exception of PROTECTED_DIRECTORY_EXCEPTIONS) {
    let normEx = exception.replace(/\\/g, "/").replace(/\/+$/, "");
    if (_IS_WINDOWS) normEx = normEx.toLowerCase();
    if (relKey === normEx || tobariSession.isDirPrefix(relKey, normEx)) {
      return null;
    }
  }

  // Check protected directories
  for (const protDir of PROTECTED_DIRECTORIES) {
    let normDir = protDir.replace(/\\/g, "/").replace(/\/+$/, "");
    if (_IS_WINDOWS) normDir = normDir.toLowerCase();
    if (tobariSession.isDirPrefix(relKey, normDir)) {
      return makeDenyResponse(
        t("gate.protected_directory"),
        t("gate.protected_detail", { filePath, protDir }),
        t("gate.protected_recovery"),
        toolName,
      );
    }
  }
...
function handler(data) {
  ...
  if (session) {
    ...
    if (agentType) {
      ...
      if (!permission.allowed) {
        return makeDenyResponse(
          ...
          `${permission.reason}`,
          t("gate.agent_policy_recovery", { agentType }),
          toolName,
        );
      }
    }
    ...
    if (toolName === "Edit" || toolName === "Write" || toolName === "NotebookEdit") {
      ...
      // 0. Protected directory check (A1)
      let result = checkProtectedDirectory(filePath, toolName);
      if (result) return result;

      // 1. Scope check
      result = checkScope(filePath, toolName);
      if (result) return result;
...
module.exports = {
  ...
  PROTECTED_DIRECTORY_EXCEPTIONS,
  DESIGN_INDICATORS,
